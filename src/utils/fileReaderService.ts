...
export const readPDFFile = async (file: File): Promise<FileReadResult> => {
  try {
    ...
    const msg = /worker|Cannot load script|fake worker/i.test(
      String((error as Error)?.message || error),
    )
      ? "Le worker PDF.js n'a pas pu être chargé. Vérifie la configuration du worker."
...
export const isFileSupported = (file: File): boolean => {
  ...
  return (
    supportedExt.includes(ext) || supportedTypes.some((t) => type.includes(t))
  );
};
